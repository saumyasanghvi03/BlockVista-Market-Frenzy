...
DIFFICULTY_LEVELS = {
    ...
    2: {
        "name": "Level 2: Intermediate Challenge", 
        "winning_criteria": {
            "min_return": 8.0,
            "max_drawdown": -7.0,
            "min_assets": 4
...
class GameState:
    ...
    def apply_difficulty_settings(self):
        ...
        if self.difficulty_level in DIFFICULTY_LEVELS:
            level_config = DIFFICULTY_LEVELS[self.difficulty_level]
            self.volatility_multiplier = level_config["market_volatility"]
...
def render_sidebar():
    ...
    if st.session_state.get('role') == 'admin':
        ...
        # Difficulty level selection with descriptions
        st.sidebar.markdown("---")
        st.sidebar.subheader("🎯 Difficulty Level")
        
        for level_num, level_config in DIFFICULTY_LEVELS.items():
            with st.sidebar.expander(f"{level_config['name']}", expanded=(level_num == game_state.difficulty_level)):
                criteria = level_config["winning_criteria"]
                st.write(f"**Winning Criteria:**")
                st.write(f"• Min Return: {criteria['min_return']}%")
                st.write(f"• Max Drawdown: {criteria['max_drawdown']}%") 
                st.write(f"• Min Assets: {criteria['min_assets']}")
                st.write(f"**Market Conditions:**")
                st.write(f"• Volatility: {level_config['market_volatility']}x")
                st.write(f"• Margin Requirement: {level_config['margin_requirement']*100}%")
                
                if st.button(f"Set Level {level_num}", key=f"level_{level_num}", disabled=(game_state.game_status == "Running")):
                    game_state.difficulty_level = level_num
                    game_state.apply_difficulty_settings()
                    st.success(f"Difficulty set to {level_config['name']}")
                    st.rerun()

        # Manual overrides (hidden in expander)
        ...
        if st.sidebar.button("▶️ Start Game", type="primary"):
            if game_state.players:
                ...
                # Apply difficulty settings when starting
                game_state.apply_difficulty_settings()
...
def render_main_interface(prices):
    ...
    # Single container approach with proper cleanup
    ...
    with main_container:
        ...
        if not hasattr(st.session_state, 'tone_js_injected'):
            st.components.v1.html('<script src="https://cdnjs.cloudflare.com/ajax/libs/tone/14.7.77/Tone.js"></script>', height=0)
            st.session_state.tone_js_injected = True

        # Safe time calculation
        remaining_time = 0
        if game_state.game_status == "Running":
            if hasattr(game_state, 'round_duration_seconds') and hasattr(game_state, 'game_start_time'):
            ...
            current_level = DIFFICULTY_LEVELS.get(game_state.difficulty_level, DIFFICULTY_LEVELS[1])
            criteria = current_level["winning_criteria"]
            
            st.markdown(f"**Time Remaining: {remaining_time // 60:02d}:{remaining_time % 60:02d}** | **{current_level['name']}**")
            with st.expander("🎯 Current Level Requirements"):
                st.write(f"**To Win:**")
                st.write(f"• Minimum Return: {criteria['min_return']}%")
                st.write(f"• Maximum Drawdown: {criteria['max_drawdown']}%")
                st.write(f"• Minimum Assets: {criteria['min_assets']}")
        ...
        if st.session_state.get('role') == 'admin':
            ...
            render_player_dashboard(prices)
        else:
            render_welcome_view(prices)
...
def render_admin_dashboard(prices):
    ...
    # Safe time calculation for admin
    if game_state.game_status == "Running":
        if hasattr(game_state, 'round_duration_seconds') and hasattr(game_state, 'game_start_time'):
            remaining_time = max(0, game_state.round_duration_seconds - int(time.time() - game_state.game_start_time))
...
def render_player_dashboard(prices):
    """Render player-specific view"""
...
def render_welcome_view(prices):
    """Render welcome view for non-players"""
...
def render_global_views(prices, is_admin=False):
    # Single container for all global views
    with st.container():
...
def check_winning_criteria(player, prices):
    """Check if player meets winning criteria for current difficulty level"""
    game_state = get_game_state()
    level_config = DIFFICULTY_LEVELS.get(game_state.difficulty_level, DIFFICULTY_LEVELS[1])
    criteria = level_config["winning_criteria"]
    ...
    if player.get('value_history'):
        peak_value = max(player['value_history'])
        current_drawdown = ((peak_value - total_value) / peak_value) * 100
    else:
        current_drawdown = 0
    
    # Count distinct assets
    distinct_assets = len([sym for sym in player['holdings'] if player['holdings'][sym] != 0])
    
    # Check criteria
    meets_return = return_pct >= criteria['min_return']
    meets_drawdown = current_drawdown >= criteria['max_drawdown']  # Note: drawdown is negative, so >= means less negative
    meets_assets = distinct_assets >= criteria['min_assets']
    
    return meets_return and meets_drawdown and meets_assets, {
        'return': return_pct,
...
def render_leaderboard(prices):
    ...
    for pname, pdata in game_state.players.items():
        ...
        meets_criteria, metrics = check_winning_criteria(pdata, prices)
        
        lb.append((pname, pdata['mode'], total_value, pdata['pnl'], sharpe_ratio, meets_criteria, metrics))
    
    if lb:
        lb_df = pd.DataFrame(lb, columns=["Player", "Mode", "Portfolio Value", "P&L", "Sharpe Ratio", "Meets Criteria", "Metrics"])
        ...
        def style_leaderboard(row):
            if row['Meets Criteria']:
                return ['background-color: #90EE90'] * len(row)  # Light green for winners
            return [''] * len(row)
        
        styled_df = lb_df[["Player", "Mode", "Portfolio Value", "P&L", "Sharpe Ratio", "Meets Criteria"]].copy()
        st.dataframe(styled_df.style.apply(style_leaderboard, axis=1).format({
            "Portfolio Value": format_indian_currency, 
            "P&L": format_indian_currency, 
            "Sharpe Ratio": "{:.2f}",
            "Meets Criteria": lambda x: "✅" if x else "❌"
        }), use_container_width=True)
        
        # Show detailed metrics for current level
        current_level = DIFFICULTY_LEVELS.get(game_state.difficulty_level, DIFFICULTY_LEVELS[1])
        criteria = current_level["winning_criteria"]
        
        st.markdown(f"**🎯 Current Level Requirements: {current_level['name']}**")
        col1, col2, col3 = st.columns(3)
        col1.metric("Min Return", f"{criteria['min_return']}%")
        col2.metric("Max Drawdown", f"{criteria['max_drawdown']}%")
        col3.metric("Min Assets", f"{criteria['min_assets']}")
        ...
        if game_state.game_status == "Finished":
            ...
            # Find actual winners (those who meet criteria)
            winners = lb_df[lb_df['Meets Criteria']]
            if not winners.empty:
                ...
                st.info(f"**Level Completion:** {len(winners)} out of {len(lb_df)} players met the winning criteria!")
                ...
                st.warning("No players met the winning criteria for this level! Try again with better strategy.")
    ...
    for name, player in game_state.players.items():
        ...
        for _ in range(3):
            ...
            if active_algo in player.get('custom_algos', {}):
                ...
                if prices.get(trade_symbol, 0) == 0: 
                    continue

                if active_algo == "Momentum Trader" and abs(price_change / prices[trade_symbol]) > 0.001:
                    if execute_trade(name, player, "Buy" if price_change > 0 else "Sell", trade_symbol, 1, prices, is_algo=True): 
                        break
                elif active_algo == "Mean Reversion" and abs(price_change / prices[trade_symbol]) > 0.001:
                    if execute_trade(name, player, "Sell" if price_change > 0 else "Buy", trade_symbol, 1, prices, is_algo=True): 
                        break
...
def main():
    ...
    # Use a single main container for the entire interface
    main_container = st.container()
    ...
    # Control refresh rate more carefully - only refresh when needed
